 ...
 namespace Valvrave_Sharp.Plugin
{
    ...
    internal class Kennen : Program
    {
        ...
        private static List<AIHeroClient> GetWTarget
            =>
                ...
                if (R.IsReady())
                {
                    var target =
                        Variables.TargetSelector.GetTargets(R.Range + 50, R.DamageType)
                            ...
                            .ToList();
                    if (target.Count > 0
                        && ((target.Count > 1 && target.Any(i => i.Health + i.MagicShield <= R.GetDamage(i)))
                            || (target.Count > 1
                ...
                if (target.Count > 0)
                {
                    if (haveR)
                    {
                        if ((target.Count(i => HaveW(i, true)) > 1
                             || target.Any(i => i.Health + i.MagicShield <= W.GetDamage(i, DamageStage.Empowered))
        ...
        private static void KillSteal()
        {
            if (MainMenu["KillSteal"]["Q"] && Q.IsReady())
            {
                ...
                if (target != null && target.Health + target.MagicShield <= Q.GetDamage(target)
                    && Q.Casting(
                        target,
                        false,
            ...
            if (MainMenu["KillSteal"]["W"] && W.IsReady()
                && GetWTarget.Any(i => i.Health + i.MagicShield <= W.GetDamage(i, DamageStage.Empowered)))
            {
                W.Cast();
            }
        }
