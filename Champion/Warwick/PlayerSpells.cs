...
using EloBuddy; namespace Warwick
{
    internal class PlayerSpells
    {
        ...
        public static void Initialize()
        {
            ...
            {
                menu.AddItem(
                    new MenuItem("Spells.Smite.Enemy", "Use Smite for Enemy!").SetValue(new KeyBind(
                        "U".ToCharArray()[0], KeyBindType.Toggle)));
                menu.AddItem(
                    new MenuItem("Spells.Smite.Monster", "Use Smite for Monsters!").SetValue(
        ...
        {
            if (!menu.Item("Spells.Smite.Enemy").GetValue<KeyBind>().Active)
                return;
        ...
        {
            if (!menu.Item("Spells.Smite.Monster").GetValue<KeyBind>().Active)
                return;

            if (Program.Player.Spellbook.CanUseSpell(SmiteSlot) != SpellState.Ready)
                return;
