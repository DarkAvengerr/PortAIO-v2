...
namespace HeavenStrikeAzir
{
    public static class AzirHarass
    {
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (Program._q.LSIsReady() && OrbwalkCommands.CanMove() && Program.qharass && (!Program.donotqharass || (!Soldiers.enemies.Any() && !Soldiers.splashautoattackchampions.Any())))
            {
            ...
            if (Program._w.LSIsReady() && OrbwalkCommands.CanMove() && !Soldiers.enemies.Any() && Program.wharass)
            {
            ...
            if (Program._w.LSIsReady() && OrbwalkCommands.CanMove() && !Soldiers.enemies.Any() && !Soldiers.soldier.Any() && Program.wharass && Program.Qisready())
            {
                var target = TargetSelector.GetTarget(Program._w.Range + 300, TargetSelector.DamageType.Magical);
