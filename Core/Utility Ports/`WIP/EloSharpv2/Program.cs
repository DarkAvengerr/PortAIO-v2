 ...
 namespace EloSharp_V2
{
    ...
    public class EloSharp
    {
        ...
        public static List<AIHeroClient> champlist = new List<AIHeroClient>();
        ...
        private static void GetHeroHandles(string setwebsite)
        {
            if (setwebsite == "opgg")
            {
                ...
                {
                    AIHeroClient herohandle = HeroManager.AllHeroes.Find(o => o.Name.ToLower().Equals(infoloading.Name.ToLower()));
                    if (herohandle != null)
                    {
        ...
        private static void Game_OnWndProc(WndEventArgs args)
        {
            if ((args.Msg == (uint)WindowsMessages.WM_LBUTTONDOWN) && mouseonload() && !Misc.Config.Item("OnlyKeyShow").GetValue<bool>())
            {
                if (!disabletext)
                {
                    // Chat.Print("Disabling");
                    disabletext = true;
                }

                else if (disabletext)
                {
                    // Chat.Print("Showing now");
                    disabletext = false;
                }
