    ...
    internal class CameraMovement : Program
    {
        public static void SemiDynamic(Vector3 position)
        {
            ...
            switch (_config.Item("dynamicmode").GetValue<StringList>().SelectedIndex)
            {
                ...
                case 1:
                    {
                        var direction = (position.To2D() - Camera.ScreenPosition).Normalized() * (_config.Item("followcurspeed").GetValue<Slider>().Value);
                        Camera.ScreenPosition = direction + Camera.ScreenPosition;
                    }
                    break;
                case 2:
                    {
                        var direction = (position.To2D() - Camera.ScreenPosition).Normalized() * (_config.Item("followtfspeed").GetValue<Slider>().Value);
                        Camera.ScreenPosition = direction + Camera.ScreenPosition;
                    }
                    break;
            }
