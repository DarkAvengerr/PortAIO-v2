 ...
 namespace Challenger_Series.Utils
{
    public class VectorHelper
    {
        ...
        {
            var objects = predefinedObjectList != null ? predefinedObjectList.ToList() : ObjectManager.Get<Obj_AI_Base>().Where(o => o.IsValidTarget(ObjectManager.Player.GetRealAutoAttackRange())).ToList();
            var apexPoint = ObjectManager.Player.ServerPosition.ToVector2() + (ObjectManager.Player.ServerPosition.ToVector2() - Game.CursorPos.ToVector2()).Normalized() * ObjectManager.Player.GetRealAutoAttackRange();
