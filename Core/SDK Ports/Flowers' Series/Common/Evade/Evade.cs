 ...
 namespace Flowers_Series.Common.Evade
{
    ...
    internal class Evade
    {
        ...
        public static bool ForcePathFollowing = false;
        ...
        private static void Game_OnOnGameUpdate(EventArgs args)
        {
            ...
            if (GameObjects.Player.Spellbook.IsAutoAttacking && !AutoAttack.IsAutoAttack(GameObjects.Player.GetLastCastedSpell().Name))
            {
                Evading = false;
                return;
            }
            ...
            foreach (var ally in GameObjects.AllyHeroes)
            {
                if (ally.IsValidTarget(1000, false))
                {
                    ...
                    if (shieldAlly != null && shieldAlly.GetValue<MenuBool>())
                    {
