...
namespace myCommon
{
    ...
    public static class HumanizerManager
    {
        ...
        public static void AddToMenu(Menu mainMenu)
        {
            ...
            var orbKey = humanizerMenu.AddSubMenu(new Menu("Orbwalker Key", "Orbwalker Key"));
            {
                orbKey.AddItem(new MenuItem("ComboKey", "Combo Key", true).SetValue(new KeyBind(32, KeyBindType.Press)));
                orbKey.AddItem(new MenuItem("LCKey", "LaneClear Key", true).SetValue(new KeyBind('V', KeyBindType.Press)));
            }
        ...
        private static bool Move()
        {
            ...
            if (humanizerMenu.Item("ComboKey", true).GetValue<KeyBind>().Active)
            {
            ...
            if (humanizerMenu.Item("LCKey", true).GetValue<KeyBind>().Active)
            {
                var haveMinions =
