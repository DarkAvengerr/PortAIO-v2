 ...
 namespace MidlaneSharp
{
    public partial class Orianna
    {
        ...
        {          
            ...
            private ConcurrentQueue<Tuple<Command, AIHeroClient>> WorkQueue;
            ...
            public delegate void dOnProcessCommand(Command cmd, AIHeroClient target);
            public event dOnProcessCommand OnProcessCommand;
            public BallMgr()
            {
                WorkQueue = new ConcurrentQueue<Tuple<Command, AIHeroClient>>();
            ...
            public void Post(Command cmd, AIHeroClient t)
            {
                WorkQueue.Enqueue(new Tuple<Command, AIHeroClient>(cmd, t));
            }

            public void Process(int count = 1)
            {
                Tuple<Command, AIHeroClient> cmd;
            ...
            private void AIHeroClient_OnCreate(GameObject sender, EventArgs args)
            {
