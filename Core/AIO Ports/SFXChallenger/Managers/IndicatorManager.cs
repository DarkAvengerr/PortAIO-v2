...
using EloBuddy; namespace SFXChallenger.Managers
{
    public class IndicatorManager
    {
        ...
        private static readonly Dictionary<string, Func<AIHeroClient, float>> Functions =
            new Dictionary<string, Func<AIHeroClient, float>>();
        ...
        public static void Add(string name, Func<AIHeroClient, float> calcDamage, bool enabled = true)
        {
            try
            {
                if (_menu == null)
        ...
        private static float CalculateDamage(AIHeroClient target)
        {
            var damage = 0f;
            try
            {
