 ...
 namespace myWorld.Library.SimpleEvade
{
    ...
    public class Skillshot
    {
        ...
        {
            if (Config.Menu.Item("ChooseMode").GetValue<KeyBind>().Active)
            {
                if (Config.Menu.Item("DangerLevel" + SpellData.MenuItemName).GetValue<Slider>().Value <= RandomValue && ObjectManager.Player.IsHPLow(Config.Menu.GetSlice("ChaseMode.MinHP")))
                {
                    return 0;
                ...
                {
                    return Config.Menu.Item("DangerLevel" + SpellData.MenuItemName).GetValue<Slider>().Value;
                }
            }
            else
            {
        ...
        public bool IsSafeToBlink(Vector2 point, int timeOffset, int delay = 0)
        {
            ...
            if (IsSafe(Initialization.PlayerPosition))
            {
                return true;
            }
            ...
            if (SpellData.Type == SkillShotType.SkillshotMissileLine)
            {
                ...
                var myPositionProjection = Initialization.PlayerPosition.ProjectOn(Start, End);
