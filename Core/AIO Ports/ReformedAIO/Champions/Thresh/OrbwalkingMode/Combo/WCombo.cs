...
namespace ReformedAIO.Champions.Thresh.OrbwalkingMode.Combo
{
    ...
    internal sealed class WCombo : OrbwalkingChild
    {
        ...
        private void OnUpdate(EventArgs args)
        {
            if (!CheckGuardians()
                || Target == null
                || Menu.Item("Thresh.Combo.W.Mana").GetValue<Slider>().Value > ObjectManager.Player.ManaPercent)
            {
                return;
            }
            ...
            if (allies == null || (Menu.Item("Thresh.Combo.W.Hooked").GetValue<bool>() && !Target.HasBuff("ThreshQ") && allies.Distance(Target) > ObjectManager.Player.Distance(Target)))
            {
                return;
            }
        ...
        protected override void OnLoad(object sender, FeatureBaseEventArgs eventArgs)
        {
            ...
            Menu.AddItem(new MenuItem("Thresh.Combo.W.Safety", "Use To Rescue Ally").SetValue(true));

            Menu.AddItem(new MenuItem("Thresh.Combo.W.Ganked", "Use If Ganked").SetValue(true));

            Menu.AddItem(new MenuItem("Thresh.Combo.W.Hooked", "Use If Enemy Hooked").SetValue(true));

            Menu.AddItem(new MenuItem("Thresh.Combo.W.Mana", "Min Mana %").SetValue(new Slider(0, 0, 100)));
        }
    }
}
