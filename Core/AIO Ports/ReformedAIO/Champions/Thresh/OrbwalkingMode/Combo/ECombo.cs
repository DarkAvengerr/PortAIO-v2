...
namespace ReformedAIO.Champions.Thresh.OrbwalkingMode.Combo
{
    ...
    internal sealed class ECombo : OrbwalkingChild
    {
        ...
        private void OnUpdate(EventArgs args)
        {
            if (!CheckGuardians()
                || Target == null
                || Menu.Item("Thresh.Combo.E.Mana").GetValue<Slider>().Value > ObjectManager.Player.ManaPercent)
            {
                return;
            }

            if (Menu.Item("Thresh.Combo.E.Push").GetValue<bool>() && Dangerous(Target))
            {
                spell.Spell.Cast(Target.Position);
            }

            if (Menu.Item("Thresh.Combo.E.Health").GetValue<Slider>().Value <= Target.HealthPercent || Target.UnderTurret(true))
            {   
        ...
        protected override void OnLoad(object sender, FeatureBaseEventArgs eventArgs)
        {
            ...
            Menu.AddItem(new MenuItem("Thresh.Combo.E.Push", "Push If Dangerous").SetValue(true));

            Menu.AddItem(new MenuItem("Thresh.Combo.E.Health", "Pull If X Health < ").SetValue(new Slider(60, 0, 100)));

            Menu.AddItem(new MenuItem("Thresh.Combo.E.Mana", "Min Mana %").SetValue(new Slider(0, 0, 100)));
        }
    }
}
