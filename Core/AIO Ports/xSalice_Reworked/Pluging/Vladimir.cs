 ...
 namespace xSaliceResurrected_Rework.Pluging
{
    ...
    internal class Vladimir : Champion
    {
        public Vladimir()
        {
            Chat.Print("Vladimir Need to Rewrite, please use others assembly!");
        ...
        {
            foreach (AIHeroClient target in ObjectManager.Get<AIHeroClient>().Where(x => x.IsValidTarget(1300)).OrderByDescending(GetComboDamage))
            {
