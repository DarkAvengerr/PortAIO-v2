        ...
        public class Orbwalker
        {
                ...
                if (ActiveMode == OrbwalkingMode.LaneClear || (ActiveMode == OrbwalkingMode.Combo && Program.ComboMenu.Item("MinionOrbwalking").GetValue<bool>()))
                {
                    if (!ShouldWait())
                    {
                        ...
                        result = (from minion in
                                      ObjectManager.Get<Obj_AI_Minion>()
                                          .Where(minion => minion.IsValidTarget() && InAutoAttackRange(minion) && minion.CharData.BaseSkinName != "gangplankbarrel" && minion.IsHPBarRendered && minion.IsHPBarRendered && minion.IsTargetable && !minion.IsDead)
                                  let predHealth =
                                      HealthPrediction.LaneClearHealthPrediction(
                                          minion, (int)((Player.AttackDelay * 1000) * LaneClearWaitTimeMod), FarmDelay)
                                  where
            ...
            private void DrawingOnOnDraw(EventArgs args)
            {
                    ...
                    && _config.Item("AACircle3").GetValue<Circle>().Active)
                {
                    ...
                    if (myTarget != null)
                    {
                        Render.Circle.DrawCircle(myTarget.Position,  myTarget.IsValid<AIHeroClient>() ? GetRealAutoAttackRange(myTarget) : 75,
