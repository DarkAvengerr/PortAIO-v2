 ...
 namespace FloraGosYasuo
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config = new Menu("GosuMechanicsYasuo_Rebirth", "Yasuo", true);
            Chat.Print("GosuMechanics Yasuo Rebirth Loaded!");
            Chat.Print("E skills Will quickly see not clear");
            Chat.Print("By:tulisan69");
            //Orbwalker
            ...
            Config.AddSubMenu(new Menu("Windwall on Combo Whitelist Setting", "ww"));
			...
			//Develop Bug
			Config.AddSubMenu(new Menu("AAAA", "Develop Bug"));
            Config.SubMenu("DevelopBug").AddItem(new MenuItem("PotsCombo", "Fast Combo")).SetValue(false);
            Config.SubMenu("DevelopBug").AddItem(new MenuItem("PostQ/E", "Fast Q/E")).SetValue(true);
            Config.SubMenu("DevelopBug").AddItem(new MenuItem("LoopholeQ", "BugQ")).SetValue(true);
            //Packets
			Config.AddSubMenu(new Menu("Packets", "Packets"));
	        Config.SubMenu("Packets").AddItem(new MenuItem("Packets", "attack rate 1.2 true")).SetValue(false);
			//Author
			Config.AddSubMenu(new Menu("Name：GosuMechanicsYasuo_ Rebirth", "Author"));
			//Versions
			Config.AddSubMenu(new Menu("版本：1.0.0.1", "Versions"));
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (!IsDashing && Config.Item("AutoQHarass").GetValue<KeyBind>().Active && Orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Combo)
            {
                ...
                if (TsTarget != null && TsTarget.BaseSkinName != "gangplankbarrel" && Config.Item("HarassTower").GetValue<bool>() && Q3.IsReady() && Config.Item("HarassQ3").GetValue<bool>() && !IsDashing && Q3READY() && Q3.IsInRange(TsTarget))
                {
                    CastQ3(TsTarget);
                }
                else if (TsTarget != null && !Config.Item("HarassTower").GetValue<bool>() && TsTarget.BaseSkinName != "gangplankbarrel" && !UnderTower(myHero.ServerPosition.To2D()) && Q3.IsReady() && Config.Item("HarassQ3").GetValue<bool>() && !IsDashing && Q3READY() && Q3.IsInRange(TsTarget))
                {
                    CastQ3(TsTarget);
                }
            }
            ...
            if (!IsDashing && Config.Item("AutoQHarass").GetValue<KeyBind>().Active && Orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Combo)
            {
                ...
                if (TsTarget != null && TsTarget.BaseSkinName != "gangplankbarrel" && Config.Item("HarassTower").GetValue<bool>() && !Q3READY() && Q.IsReady() && Config.Item("HarassQ").GetValue<bool>() && !IsDashing && Q.IsInRange(TsTarget))
                {
                    CastQ12(TsTarget);
                }
                else if (TsTarget != null && TsTarget.BaseSkinName != "gangplankbarrel" && !Config.Item("HarassTower").GetValue<bool>() && !Q3READY() && Q.IsReady() && Config.Item("HarassQ").GetValue<bool>() && !IsDashing && !UnderTower(myHero.ServerPosition.To2D()) && Q.IsInRange(TsTarget))
                {
                    CastQ12(TsTarget);
                }
            }
        ...
        public static void Harass()
        {
            ...
            if (TsTarget == null || TsTarget.BaseSkinName == "gangplankbarrel")
            {
                return;
            }
        ...
        public static void AutoQ()
        {
            ...
            foreach (var minion in Qminion.Where(minion => minion.IsValidTarget(Q.Range) && !minion.IsDead))
            {
                if (minion == null && minion.BaseSkinName == "gangplankbarrel")
                {
                    return;
                }
                if (!Q3READY() && Config.Item("AutoQ1").GetValue<bool>() && minion.IsValidTarget(Q.Range) && !IsDashing)
