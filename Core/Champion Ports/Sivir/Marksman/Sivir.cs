 ...
 namespace Marksman.Champions
{
    ...
    internal class Sivir : Champion
    {
        ...
        public void AIHeroClient_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (sender.IsEnemy && sender is AIHeroClient && args.Target.IsMe && E.IsReady())
            {
                foreach (
                    var c in
            ...
            if (((AIHeroClient) sender).ChampionName.ToLower() == "vayne" &&
                args.SData.Name == ((AIHeroClient) sender).GetSpell(SpellSlot.E).Name)
            {
                for (var i = 1; i < 8; i++)
                {
                    var championBehind = ObjectManager.Player.Position +
                                         Vector3.Normalize(((AIHeroClient) sender).ServerPosition -
