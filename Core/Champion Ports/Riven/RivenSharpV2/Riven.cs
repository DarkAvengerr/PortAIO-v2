            ...
            else if (dist < (target.AttackRange + target.BoundingRadius+150))
            {
                Obj_AI_Turret closest_tower =
                           ObjectManager.Get<Obj_AI_Turret>()
                               ...
                               .First();
                EloBuddy.Player.IssueOrder(GameObjectOrder.MoveTo, closest_tower.Position);
                if (E.IsReady())
                {
            ...
            else if (getQJumpCount() == 1 && !W.IsReady())
            {
                Obj_AI_Turret closest_tower =
                          ObjectManager.Get<Obj_AI_Turret>()
                              ...
                              .OrderBy(tur => tur.Distance(Player.Position))
                              .First();
                EloBuddy.Player.IssueOrder(GameObjectOrder.MoveTo, closest_tower.ServerPosition);
