...
namespace Shen.Modes
{
    internal class ModeUlti
    {
        ...
        public static void Initialize(LeagueSharp.Common.Menu menuConfig)
        {
            ...
            //List<AIHeroClient> allyList = HeroManager.Allies.Where(a => !a.IsMe).ToList();

            //for (int i = 0; i < allyList.Count; i++)
            //{
            //    strImportantAlly[i + 1] = allyList[i].BaseSkinName;
            //}
       ...
       private static void Drawing_OnDraw(EventArgs args)
        ...
        {
            if (LeagueSharp.Common.Utils.TickCount - lastPingTickCount < 30 * 1000)
            {
                return;
            }

            lastPingTickCount = LeagueSharp.Common.Utils.TickCount;
            pingLocation = position;
            SimplePing();

            LeagueSharp.Common.Utility.DelayAction.Add(200, SimplePing);
