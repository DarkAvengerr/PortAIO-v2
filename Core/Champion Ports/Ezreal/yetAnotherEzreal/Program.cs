...
namespace yetAnotherEzreal
{
    ...
    "Akali", "Diana", "Fiddlesticks", "Fiora", "Fizz", "Heimerdinger", "Jayce",
        ...
        {
            Chat.Print("yetAnotherEzreal by FlapperDoodle, version: " + VersionNumber);
            if (ObjectManager.Player.ChampionName != "Ezreal")
            {
                Chat.Print("Please use Ezreal~");
                return;
            }
            ...
            if (LeagueSharp.Common.Utility.Map.GetMap().Type == LeagueSharp.Common.Utility.Map.MapType.SummonersRift ||
                LeagueSharp.Common.Utility.Map.GetMap().Type == LeagueSharp.Common.Utility.Map.MapType.HowlingAbyss)
        ...
        private static void Orbwalking_OnTargetChange(AttackableUnit oldtarget, AttackableUnit newtarget)
        {
            if (Config.Item("Muramana-Enabled").GetValue<bool>() && Config.Item("Muramana-Mana").GetValue<Slider>().Value <= (Player.Mana / Player.MaxMana) * 100 &&
                ...
                Muramana.IsReady() && newtarget.IsValid && newtarget.Type == GameObjectType.AIHeroClient) || (newtarget.IsValid && newtarget.Type != GameObjectType.AIHeroClient && Player.HasBuff("Muramana") && Muramana.IsOwned() &&
            ...
            ObjectManager.Get<Obj_AI_Minion>()
                .Any(
                    minion =>
                        ...
                        (minion.BaseSkinName == "SRU_ChaosMinionSiege" | minion.BaseSkinName == "SRU_OrderMinionSiege" |
                        minion.BaseSkinName == "SRU_ChaosMinionSuper" | minion.BaseSkinName == "SRU_OrderMinionSuper"))) &&
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (Config.Item("Zhonya").GetValue<bool>() && Items.HasItem(Zhonya.Id) && Zhonya.IsReady()
                ...
                && (ObjectManager.Get<AIHeroClient>().Any(enemy => enemy.IsEnemy && enemy.IsValidTarget(1600)) || Player.HasBuffOfType(BuffType.Damage) || Player.HasBuffOfType(BuffType.Poison)))
                ...
                if (wHit.Hitchance >= ComboHitChance)
                {
                    ...
                    if (Config.Item("Combo-Use-E-W").GetValue<bool>() && _e.IsReady())
                    {
                        ...
                        if (!landingPosition.UnderTurret(true) && !landingPosition.IsWall() && //Check if landing position is definitely not a wall or a turret
                            ObjectManager.Get<AIHeroClient>().Count(enemy => enemy.IsEnemy && enemy.IsValidTarget(600, true, target.Position)) - 1 //The number of enemies, other than the target, near the target
                            <= ObjectManager.Get<AIHeroClient>().Count(ally => ally.IsAlly && !ally.IsDead && ally.Distance(target) < 600) && //The number of allies near the target
        ...
        private static void CastOffensiveItems(AIHeroClient target)
        {
                                    ...
                                    Config.Item("Harass-" + enemy.ChampionName).GetValue<bool>()))
            {
                ...
                for (var i = 1; i < 3; i++)
                {
                    ...
                    if (pointTo.IsWall() || pointTo.UnderTurret(true) ||
                        ObjectManager.Get<AIHeroClient>().Any(enemy => enemy != enemyInRange && enemy.IsValidTarget(300, true, pointTo))) continue;
        ...
        {
            foreach (var enemyInRange in ObjectManager.Get<AIHeroClient>().Where(enemy => enemy.IsValidTarget(_q.Range)))
            {
                         ...
                         (qDamage + wDamage > enemyInRange.Health))
                {
                    ...
                    _w.Cast(enemyInRange);
                         ...
                         ObjectManager.Player.Spellbook.CanUseSpell(Ignite) == SpellState.Ready && Player.GetSummonerSpellDamage(enemyInRange, Damage.SummonerSpell.Ignite) > enemyInRange.Health)
                {
                    ObjectManager.Player.Spellbook.CastSpell(Ignite, enemyInRange);
                }
            }
        }
