 ...
 namespace Orianna_by_trelli
{
    class Program
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (qValue.Active)
                LeagueSharp.Common.Utility.DrawCircle(ObjectManager.Player.Position, Q.Range,
                    qValue.Color);

            var wValue = Config.Item("DrawWRange").GetValue<Circle>();
            if (wValue.Active)
                LeagueSharp.Common.Utility.DrawCircle(BallManager.CurrentBallDrawPosition, W.Width,
                    wValue.Color);

            var eValue = Config.Item("DrawERange").GetValue<Circle>();
            if (eValue.Active)
                LeagueSharp.Common.Utility.DrawCircle(ObjectManager.Player.Position, E.Range,
                    eValue.Color);

            var rValue = Config.Item("DrawRRange").GetValue<Circle>();
            if (rValue.Active)
            {
                LeagueSharp.Common.Utility.DrawCircle(BallManager.CurrentBallDrawPosition, R.Width, rValue.Color);
            }
        }
        #endregion
        ...
        private static int GetNumberHitByR(Obj_AI_Base target)
        {
            ...
            if (debug) { Chat.Print("Hitbox size is: " + target.BoundingRadius); };
            int totalHit = 0;
            foreach (AIHeroClient current in ObjectManager.Get<AIHeroClient>())
            {
                var prediction = R.GetPrediction(current, true);
                if (debug) { Chat.Print("HitChance is: " + prediction.Hitchance); };
            ...
            if (debug) { Chat.Print("Targets hit is: " + totalHit); };
            return totalHit;
        }
        ...
        private static Tuple<Vector3, int> getMECQPos(AIHeroClient target)
        {
        ...
        private static bool isAlone(AIHeroClient target)
        {
            int numEnemies = 0;
            foreach (AIHeroClient current in ObjectManager.Get<AIHeroClient>())
            {
        ...
        #region Farming
        private static void FarmWTarget(AIHeroClient target)
        {
