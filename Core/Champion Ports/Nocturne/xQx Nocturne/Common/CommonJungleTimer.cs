 ...
 namespace Nocturne.Common
{
    internal class CommonJungleTimer
    {
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if ((int) Game.Time - nextTime >= 0)
            {
                nextTime = (int) Game.Time + 1;
                ...
                foreach (JungleCamp jungle in junglesDead)
                {
                    ...
                    jungle.NextRespawnTime = (int) Game.Time + jungle.RespawnTime;
                }

                foreach (JungleCamp jungleCamp in
                    JungleMobs.Where(jungleCamp => (jungleCamp.NextRespawnTime - (int) Game.Time) <= 0))
                {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                int timeClock = jungleCamp.NextRespawnTime - (int) Game.Time;
