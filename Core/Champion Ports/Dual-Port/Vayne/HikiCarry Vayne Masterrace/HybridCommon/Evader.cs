 ...
 namespace HybridCommon
{
    ...
    public class Evader
    {
        ...
        public Evader(out Menu _evade, EvadeMethods method = EvadeMethods.None, Spell spl = null)
        {
            ...
            Chat.Print("<font color='#ff3232'>HikiCarry Vayne Masterrace: </font><font color='#d4d4d4'>Evader loaded! You can rekt everyone with this assembly 1.337 ms.</font>", ObjectManager.Player.ChampionName);
            Chat.Print(string.Format("<font color='#ff3232'>HikiCarry Vayne Masterrace - </font><font color='#d4d4d4'>Assembly Version: </font><font color='#ff3232'><b>{0}</b></font> ", typeof(Program).Assembly.GetName().Version));
        }
        ...
        {
            if (evade != null && evade.Item("EVADEENABLE").GetValue<bool>() && sender.Type == GameObjectType.AIHeroClient)
            {
                if (sender.IsEnemy)
                {
