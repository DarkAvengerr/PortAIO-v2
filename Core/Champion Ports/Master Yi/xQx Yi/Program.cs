 ...
 namespace MasterYiQx
{
    internal class Program
    {
        ...
        static void Game_OnGameLoad()
        {
            ...
            foreach (var xEnemy in ObjectManager.Get<AIHeroClient>().Where(enemy => enemy.IsEnemy))
            {
                AIHeroClient enemy = xEnemy;
                    ...
                    )
                {
                    switch (spell.SkillType)
                    {
                        case SkillShotType.SkillshotTargeted:
                            if (Q.IsReady())
                            {
                                Chat.Print("SkillShotType.SkillshotTargeted");
                        ...
                        case SkillShotType.SkillshotCircle:
                            ...
                            {
                                Chat.Print("SkillShotType.SkillshotCircle");
                                if (Q.IsReady())
                                {
                                    var t = TargetSelector.GetTarget(Q.Range, TargetSelector.DamageType.Physical);
                                    if (t.IsValidTarget())
                        ...
                        case SkillShotType.SkillshotLine:
                            ...
                            {
                                Chat.Print("SkillShotType.SkillshotLine");
                                if (Q.IsReady())
                                {
                                    var t = TargetSelector.GetTarget(Q.Range, TargetSelector.DamageType.Physical);
                                    if (t.IsValidTarget())
                        ...
                        case SkillShotType.SkillshotUnknown:
                            ...
                            {
                                Chat.Print("SkillShotType.SkillshotUnknown");
                                if (Q.IsReady())
                                {
                                    var t = TargetSelector.GetTarget(Q.Range, TargetSelector.DamageType.Physical);
                                    if (t.IsValidTarget())
            ...
            if (Config.Item("HealUseW").GetValue<KeyBind>().Active)
            {
                foreach (var enemy in ObjectManager.Get<AIHeroClient>().Where(enemy => !enemy.IsAlly))
                {
                    if (Player.Health > enemy.Health && Player.Level >= enemy.Level)
                        return;
                    if (Player.Distance(enemy) > Q.Range + 200 && enemy.IsValid && enemy.IsHPBarRendered)
                        return;
                }
        ...
        public static void UseItems(AIHeroClient vTarget, bool useNonTargetedItems = false)
        {
            if (vTarget == null)
                return;
