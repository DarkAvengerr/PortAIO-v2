...
{
    internal class Viktor : Abstracts.Champion
    {
        ...
        protected override void Combo()
        {
            ...
            if (r)
            {
                ...
                {
                    if (Orbwalking.InAutoAttackRange(qTarget) && !Player.Spellbook.IsAutoAttacking)
                    {
                        EloBuddy.Player.IssueOrder(GameObjectOrder.AttackUnit, qTarget);
                    }
                    else
                    {
        ...
        protected override void Harass()
        {
            ...
            if (Menu.Item(Menu.Name + ".harass.q").GetValue<bool>())
            {
                ...
                if (target != null)
                {
                    ...
                    {
                        _lastQCast = Game.Time;
                        if (Orbwalking.InAutoAttackRange(target) && !Player.Spellbook.IsAutoAttacking)
                        {
        ...
        private void RFollowLogic()
        {
            try
            {
                if (_lastRMoveCommand + RMoveInterval > Environment.TickCount || Player.Spellbook.IsAutoAttacking)
                {
                    return;
                }
        ...
        private bool RLogic(UltimateModeType mode, AIHeroClient target, bool simulated = false)
        {
            try
            {
                if (!R.Instance.Name.Equals("ViktorChaosStorm", StringComparison.OrdinalIgnoreCase) ||
        ...
        private void WLogic(AIHeroClient target, HitChance hitChance)
        {
            try
            {
        ...
        private bool ELogic(AIHeroClient target, List<AIHeroClient> targets, HitChance hitChance, int minHits = 1)
        {
            return ELogic(
                target, targets.Select(t => t as Obj_AI_Base).Where(t => t != null).ToList(), hitChance, minHits);
        }

        private bool ELogic(AIHeroClient mainTarget, List<Obj_AI_Base> targets, HitChance hitChance, int minHits)
        {
            try
            {
        ...
        private bool ELogicNew(AIHeroClient mainTarget, HitChance hitChance)
        {
            try
            {
                if (mainTarget == null)
