 ...
 namespace Lee_Sin.Misc
{
    ...
    class BubbaKush : LeeSin
    {
        ...
        /// </returns>
        public static
            Vector3 GetWardFlashPositions(float maxTravelDistance, AIHeroClient player, byte minHitRequirement, List<AIHeroClient> enemies)
        {
        ...
        {
            var results = new List<Tuple<Geometry.Polygon.Rectangle, byte, List<AIHeroClient>>>();
            foreach (var polygon in polygons)
            {
                byte count = 0x0;
                var inPoly = new List<AIHeroClient>();
