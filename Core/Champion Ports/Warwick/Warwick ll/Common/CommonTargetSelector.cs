 ...
 namespace WarwickII.Common
{
    ...
    internal class CommonTargetSelector
    {
        ...
        public static void Initialize(Menu ParentMenu)
        {
            ...
            MenuLocal.AddItem(
                new MenuItem("TS", "Active Target Selector:").SetValue(
                    new StringList(new[] {"Warwick Target Selector", "L# Target Selector"})))
            ...
            //foreach (var langItem in HeroManager.Enemies.Select(e =>MenuQ.AddItem(new MenuItem("enemy_" + e.ChampionName,string.Format("{0}Focus {1}", Tab, e.ChampionName)).SetValue(false)).SetTag(12)))
        ...
        {
            if (Selector != TargetSelect.Warwick)
            {
                return;
            }
