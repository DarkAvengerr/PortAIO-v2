...
namespace EasyKogMaw
{
    class EasyKogMaw : EasyLib1.Champion
    {
        ...
        protected override void Update()
        {
            ...
            {
                foreach (var enemy in ObjectManager.Get<AIHeroClient>().Where(enemy => enemy.IsEnemy && enemy.IsValid && enemy.Distance(Player) < Spells.get("R").Range))
                {
            ...
            if (Menu.Item("Auto_passive").GetValue<bool>() && Player.IsDead/*Player.HasBuff("KogMawIcathianSurprise")*/)
            {
                ...
                foreach (var enemy in ObjectManager.Get<AIHeroClient>().Where(enemy => enemy.IsEnemy && !enemy.IsDead && enemy.Distance(Player) < 1000 && enemy.Health < PassiveDamage(enemy)))
                {
                    if (target == null)
                        target = enemy;
                    else
                ...
                if (target == null)
                {
                    foreach (var enemy in ObjectManager.Get<AIHeroClient>().Where(enemy => enemy.IsEnemy && !enemy.IsDead && enemy.Distance(Player) < 1000))
                    {
                        if (target == null)
                            target = enemy;
                        else
        ...
        private void CastW()
        {
            ...
            AIHeroClient target = TargetSelector.GetTarget(Spells.get("W").Range + Player.AttackRange, TargetSelector.DamageType.Magical);
        ...
        private float PassiveDamage(AIHeroClient hero)
        {
