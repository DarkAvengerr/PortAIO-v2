...
using EloBuddy; 
using LeagueSharp.Common; 
 namespace LordsSyndra //LordZEDith Updated Orbwalker
{
    ...
    public class xSLxOrbwalker
    {
        ...
        public static void AddToMenu(Menu menu)
        {
            ...
            menuDrawing.AddItem(new MenuItem("orb_Draw_MinionHPBar", "Minion HPBar").SetValue(new Circle(false, Color.Black)));
            ...
            menu.AddItem(new MenuItem("xSLx_info2", "Credits:"));
        ...
        private static void Obj_SpellMissile_OnCreate(GameObject sender, EventArgs args)
        {
            ...
            if (sender.Name == "Azir_Base_P_Soldier_Ring.troy")
            {
                //Chat.Print("Solider Added" + sender.NetworkId);
                Soilders.Add(sender);
                //Chat.Print("" + Soilders.Count);
            }
        ...
        {
            if (CurrentMode == Mode.None || MenuGUI.IsChatOpen || CustomOrbwalkMode || MyHero.IsChannelingImportantSpell() || MyHero.HasBuff("katarinarsound"))
                return;
            CheckAutoWindUp();
        ...
        public static void Orbwalk(Vector3 goalPosition, AttackableUnit mytarget)
        {
            ...
            if (MyHero.IsMelee() && target != null && target.Distance(MyHero) < GetAutoAttackRange(MyHero, target) &&
                Menu.Item("orb_Melee_Prediction").GetValue<bool>() && target is AIHeroClient && Game.CursorPos.Distance(target.Position) < 300)
            {
