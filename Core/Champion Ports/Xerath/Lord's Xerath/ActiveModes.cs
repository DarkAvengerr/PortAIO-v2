...
namespace Lords_Xerath
{
    public class ActiveModes
    {
        ...
        private static AIHeroClient lastUltTarget = null;
            ...
            if (SpellManager.IsCastingUlt && Config.BoolLinks["ultSettingsEnabled"].Value)
            {
                switch (Config.StringListLinks["ultSettingsMode"].Value.SelectedIndex)
                {
                    ...
                    case 4:
                        ...
                        var targets = ObjectManager.Get<AIHeroClient>().Where(h => h.IsValidTarget(R.Range) && h.Distance(Game.CursorPos, true) < 500 * 500);
