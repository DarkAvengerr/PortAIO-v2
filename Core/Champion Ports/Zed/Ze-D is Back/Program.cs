    ...
    class Program
    {
            ...
            try
            {
                ...
                foreach (var e in enemy)
                {
                    ...
                    if (DangerDB.DangerousList.Any(spell => spell.Contains(rdata.SData.Name)))
                        _config.SubMenu("Misc").AddItem(new MenuItem("ds" + e.BaseSkinName, rdata.SData.Name)).SetValue(true);
                }
        ...
        private static void OnProcessSpell(Obj_AI_Base unit, GameObjectProcessSpellCastEventArgs castedSpell)
        {
            ...
            if (unit.IsEnemy)
            {
                ...
                _config.Item("ds" + unit.BaseSkinName).GetValue<bool>())
                {
        ...
        private static void Harass(AIHeroClient t)
        {
            ...
            if (target.IsValidTarget() && _config.Item("longhar").GetValue<KeyBind>().Active && _w.IsReady() && _q.IsReady() && ObjectManager.Player.Mana >
                ...
                ObjectManager.Player.Spellbook.GetSpell(SpellSlot.W).SData.Mana && target.Distance(_player.Position) > 850 &&
