...
using EloBuddy; 
 using LeagueSharp.Common; 
 namespace YorickTheStaffer.Evade
{
    internal static class SkillshotDetector
    {
        ...
        static SkillshotDetector()
        {
            ...
            if (false && Config.TestOnAllies && ObjectManager.Get<AIHeroClient>().Count() == 1)
            {
        ...
        private static void GameObject_OnDelete(GameObject sender, EventArgs args)
        {
            ...
            for (var i = yorick.Program.DetectedSkillshots.Count - 1; i >= 0; i--)
            {
                var skillshot = yorick.Program.DetectedSkillshots[i];
                if (skillshot.SpellData.ToggleParticleName != "" &&
                    sender.Name.Contains(skillshot.SpellData.ToggleParticleName))
                {
                    yorick.Program.DetectedSkillshots.RemoveAt(i);
                }
            }
        }
        ...
        private static void ObjSpellMissileOnOnDelete(GameObject sender, EventArgs args)
        {
            ...
            {
                foreach (var skillshot in yorick.Program.DetectedSkillshots)
                {
            ...
            yorick.Program.DetectedSkillshots.RemoveAll(
                skillshot =>
