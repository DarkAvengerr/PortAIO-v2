...
namespace yetAnotherUdyr
{
    public class Program
    {
        ...
        {
            Chat.Print("yetAnotherUdyr by FlapperDoodle, version: " + versionNumber);
            if (ObjectManager.Player.ChampionName != "Udyr")
            {
                Chat.Print("Please use Udyr~");
                return;
            }
        ...
        private static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            try
            {
                if (!sender.IsValid<Obj_AI_Turret>() || !args.Target.IsValid<AIHeroClient>() || !args.Target.IsMe) return;
