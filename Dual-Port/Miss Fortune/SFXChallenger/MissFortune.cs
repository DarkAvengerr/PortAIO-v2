...
{
    internal class MissFortune : Abstracts.Champion
    {
        ...
        private void OnOrbwalkingAfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            try
            {
                if (unit.IsMe)
                {
                    ...
                    {
                        useW = target is AIHeroClient && Menu.Item(Menu.Name + ".harass.w").GetValue<bool>();
                    }
                    if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
                    {
                        useW = target is AIHeroClient && Menu.Item(Menu.Name + ".combo.w").GetValue<bool>();
                    }
                    if (useW)
                    {
                        W.Cast();
        ...
        protected override void OnPreUpdate()
        {
            if (Game.Time - _lastRCast < 5 && !_lastRPosition.Equals(Vector3.Zero))
            {
                ...
                if (hits <= 0)
                {
                    ...
                    LeagueSharp.Common.Utility.DelayAction.Add(1000, delegate { BlockOrdersManager.Automatic = true; });
                }
            }
        }
