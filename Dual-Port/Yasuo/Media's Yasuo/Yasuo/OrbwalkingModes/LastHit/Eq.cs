 ...
 namespace YasuoMedia.Yasuo.OrbwalkingModes.LastHit
{
    ...
    internal class Eq : FeatureChild<LastHit>
    {
        ...
        private void ValidateDashes()
        {
            ...
            var toBeRemoved = new List<Dash>(this.Dashes.Count);

            foreach (var entry in this.Dashes)
            {
                entry.CheckKnockups();
                ...
                if (entry.MinionsHitCircular.Where(x => x.Health <= this.providerQ.GetDamage(x)).All(x => x.Equals(entry.Unit)))
                {
                    toBeRemoved.Add(entry);
                }
            }
