    ...
    public static class BadaoYasuoHelper
    {
        ...
        private static void Obj_AI_Base_OnPlayAnimation(Obj_AI_Base sender, GameObjectPlayAnimationEventArgs args)
        {
            ...
            if (args.Animation == "Spell1_Dash")
            {
                ...
                LeagueSharp.Common.Utility.DelayAction.Add(500, () => Orbwalking.Attack = true);
            }
        }
        ...
        {
            //Chat.Print(ObjectManager.Player.Distance(Game.CursorPos).ToString());
            //Chat.Print(BadaoMainVariables.Q.Instance.Name);
        }
        ...
        public static AIHeroClient GetESelector(bool underturret = false)
        {
            //underturret = BadaoYasuoVariables.DiveTurretKey.GetValue<KeyBind>().Active;
            //var Etargets = GetETargets().Where(x => !underturret ? !GetEDashEnd(x).To3D().UnderTurret(true) : x != null);
        ...
        public static bool IsOnAir(AIHeroClient target)
        {
            return target.HasBuffOfType(BuffType.Knockback) || target.HasBuffOfType(BuffType.Knockup);
        }
        public static int GetRCount(AIHeroClient target)
        {
            return
