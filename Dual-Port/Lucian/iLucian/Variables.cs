 ...
 namespace iLucian
{
    internal class Variables
    {
        ...
        {
            return ObjectManager.Player.LSHasBuff("LucianPassiveBuff");
        }
        ...
        public static float GetComboDamage(Obj_AI_Base target)
        {
            ...
            if (Spell[Spells.Q].LSIsReady())
            {
                damage += Spell[Spells.Q].GetDamage(target);
            }
            if (Spell[Spells.W].LSIsReady())
            {
                damage += Spell[Spells.W].GetDamage(target);
            }
            if (Spell[Spells.E].LSIsReady())
            {
                damage += (float) ObjectManager.Player.LSGetAutoAttackDamage(target, true) * 2;
            }
