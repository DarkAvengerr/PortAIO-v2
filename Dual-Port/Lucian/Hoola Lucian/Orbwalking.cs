 ...
 namespace HoolaLucian
{
    ...
    public static class Orbwalking
    {
        ...
        public static bool CanAttack() // Fix this later
        {
        ...
        public static bool CanMove(float extraWindup) // Fix this later
        {
            if (!Move)
            {
                return false;
            ...
            bool randomizeMinDistance = true)
        {
            ...
            try
            {
                if (target.LSIsValidTarget() && CanAttack()) // Fix this later
                {
                    DisableNextAttack = false;
                ...
                else if (CanMove(extraWindup)) // Fix this later
                {
        ...
        private static void Obj_AI_Base_OnSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args) // Fix this later
        {
            if (sender.IsMe)
            {
                if (Game.Ping <= 30) //First world problems kappa
                {
                    LeagueSharp.Common.Utility.DelayAction.Add(30, () => Obj_AI_Base_OnSpellCast_Delayed(sender, args));
                    return;
                }
        ...
        /// <param name="sender">The sender.</param>
        /// <param name="args">The <see cref="GameObjectProcessSpellCastEventArgs"/> instance containing the event data.</param>
        private static void Obj_AI_Base_OnSpellCast_Delayed(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args) // Fix this later
        {
        ...
        private static void OnProcessSpell(Obj_AI_Base unit, GameObjectProcessSpellCastEventArgs Spell) // Fix this later
        {
            try
            {
        ...
        public class Orbwalker
        {
            ...
            public virtual AttackableUnit GetTarget()
            {
                    ...
                    ActiveMode == OrbwalkingMode.LastHit)
                {
                    ...
                    foreach (var minion in MinionList)
                    {
                        var t = (int)(Player.AttackCastDelay * 1000) - 100 + Game.Ping / 2 +
                                1000 * (int)Math.Max(0, Player.LSDistance(minion) - Player.BoundingRadius) / int.MaxValue;
                ...
                /*Champions*/
                if (ActiveMode != OrbwalkingMode.LastHit && !Player.LSHasBuff("LucianR"))
                {
                    var target = TargetSelector.GetTarget(-1, TargetSelector.DamageType.Physical);
