...
using EloBuddy; namespace SurvivorBrand
{
    class Program
    {
        ...
        private static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (t.LSIsValidTarget(E.Range) && (t.LSHasBuff("brandablaze") || E.LSIsReady()))
            {
        ...
        private static void QUsage()
        {
            ...
            if (Menu.Item("QOnlyAblazed").GetValue<bool>() && m.LSHasBuff("brandablaze"))
            {
                foreach (var enemy in HeroManager.Enemies.Where(enemy => enemy.LSIsValidTarget(Q.Range) && enemy.LSHasBuff("brandablaze")))
                {
                    SebbySpell(Q, enemy);
                }
            }
            ...
            {
                if (m.LSHasBuff("brandablaze") && Menu.Item("QAblazedEnemy").GetValue<bool>())
                {
                    var spreadTarget = m;
                ...
                if (Orbwalker.ActiveMode == SebbyLib.Orbwalking.OrbwalkingMode.Combo && !W.LSIsReady() && Player.ManaPercentage() > RManaC + Q.ManaCost)
                {
                    var enemystunned = HeroManager.Enemies.Find(enemy => enemy.LSIsValidTarget(Q.Range) && enemy.LSHasBuff("stun") || enemy.IsStunned && Menu.Item("PrioritizeStunned").GetValue<bool>());
        ...
        private static void WUsage()
        {
            ...
            if (t.LSIsValidTarget())
            {
                ...
                {
                    var enemystunned = HeroManager.Enemies.Find(enemy => enemy.LSIsValidTarget(W.Range) && enemy.LSHasBuff("stun") || enemy.IsStunned && Menu.Item("PrioritizeStunned").GetValue<bool>());
                    if (enemystunned != null)
                    {
                        Orbwalker.ForceTarget(enemystunned);
        ...
        private static void EUsage()
        {
            ...
            if (t.LSIsValidTarget())
            {
                ...
                {
                    var enemystunned = HeroManager.Enemies.Find(enemy => enemy.LSIsValidTarget(E.Range) && enemy.LSHasBuff("stun") || enemy.IsStunned && Menu.Item("PrioritizeStunned").GetValue<bool>());
                    if (enemystunned != null)
                    {
                        Orbwalker.ForceTarget(enemystunned);
        ...
        private static void RUsage()
        {
            ...
            if (t2.LSIsValidTarget(R.Range) && t2.LSCountEnemiesInRange(bounceRange) >= Menu.Item("NearbyREnemies").GetValue<Slider>().Value && Menu.Item("NearbyREnemies").GetValue<Slider>().Value > 0)
                R.Cast(t2);
        ...
        private static void LaneClear()
        {
            ...
            if (Player.ManaPercentage() > Menu.Item("LaneClearManaManager").GetValue<Slider>().Value)
            {
                ...
                if (Menu.Item("laneclearE").GetValue<bool>() && E.LSIsReady() && !W.LSIsReady())
                {
                    foreach (var minion in allMinions)
                    {
                        if (minion.LSIsValidTarget() && minion.LSHasBuff("brandablaze"))
                        {
                            E.CastOnUnit(minion);
                        }
                        else
        ...
        private static float CalculateDamage(Obj_AI_Base enemy)
        {
            ...
            // bool HasABlazeBomb = enemy.LSHasBuff("brandablazebomb");
            damage += BrandPassive;
            if (enemy.LSHasBuff("brandablazebomb"))
            {
            ...
            if (W.LSIsReady())
            {
                if (enemy.LSHasBuff("brandablaze") || enemy.LSHasBuff("ablaze"))
                {
