...
public class EasyAhriCore : EasyLib2.Champion
{
    private Items.Item ItemDFG;

    public EasyAhriCore() : base("Ahri")
    {
    ...
    {
        if (BoolLinks["harass_e"].Value && GetSpellData(SpellSlot.E).SData.Mana + SliderLinks["harass_mana"].Value.Value <= Player.Mana)
            Spells.CastSkillshot(E, TargetSelector.DamageType.Magical);
        if (BoolLinks["harass_q"].Value && GetSpellData(SpellSlot.Q).SData.Mana + SliderLinks["harass_mana"].Value.Value <= Player.Mana)
            Spells.CastSkillshot(Q, TargetSelector.DamageType.Magical);
        if (BoolLinks["harass_w"].Value && GetSpellData(SpellSlot.W).SData.Mana + SliderLinks["harass_mana"].Value.Value <= Player.Mana)
            Spells.CastSelf(W, TargetSelector.DamageType.Magical);
    }

    protected override void OnAuto()
    {
        if (BoolLinks["auto_e"].Value && GetSpellData(SpellSlot.E).SData.Mana + SliderLinks["auto_mana"].Value.Value <= Player.Mana)
            Spells.CastSkillshot(E, TargetSelector.DamageType.Magical);
        if (BoolLinks["auto_q"].Value && GetSpellData(SpellSlot.Q).SData.Mana + SliderLinks["auto_mana"].Value.Value <= Player.Mana)
            Spells.CastSkillshot(Q, TargetSelector.DamageType.Magical);
        if (BoolLinks["auto_w"].Value && GetSpellData(SpellSlot.W).SData.Mana + SliderLinks["auto_mana"].Value.Value <= Player.Mana)
            Spells.CastSelf(W, TargetSelector.DamageType.Magical);
    }
    ...
    protected override void OnUpdate()
    {
        if (E.IsReady())
        {
            ...
            foreach (AIHeroClient enemy in Enemies.Where(x => Player.Distance(x, false) <= E.Range))
            {
    ...
    protected override void OnDraw()
    {
        if (BoolLinks["drawing_q"].Value)
            LeagueSharp.Common.Utility.DrawCircle(Player.Position, Q.Range, Color.FromArgb(100, 0, 255, 0));
        if (BoolLinks["drawing_w"].Value)
            LeagueSharp.Common.Utility.DrawCircle(Player.Position, W.Range, Color.FromArgb(100, 0, 255, 0));
        if (BoolLinks["drawing_e"].Value)
            LeagueSharp.Common.Utility.DrawCircle(Player.Position, E.Range, Color.FromArgb(100, 0, 255, 0));
