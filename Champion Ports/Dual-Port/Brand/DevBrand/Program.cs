    ...
    class Program
    {
        ...
        static void Game_OnGameLoad()
        {
            ...
            try
            {
                ...
                Chat.Print(string.Format("<font color='#fb762d'>DevBrand Loaded v{0}</font>", Assembly.GetExecutingAssembly().GetName().Version));
            }
            catch (Exception ex)
            {
        ...
        static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (BarrierGapCloser && gapcloser.Sender.IsValidTarget(Player.AttackRange) && Player.GetHealthPerc() < BarrierGapCloserMinHealth)
            {
                if (barrierManager.Cast())
                    Chat.Print(string.Format("OnEnemyGapcloser -> BarrierGapCloser on {0} !", gapcloser.Sender.BaseSkinName));
            }
        ...
        public static void BurstCombo()
        {
            ...
            totalManaCost += Player.Spellbook.GetSpell(SpellSlot.R).SData.Mana;
            totalManaCost += Player.Spellbook.GetSpell(SpellSlot.W).SData.Mana;
        ...
        public static void Combo()
        {
            ...
            if (igniteManager.CanKill(eTarget))
            {
                if (igniteManager.Cast(eTarget))
                    Chat.Print(string.Format("Ignite Combo KS -> {0} ", eTarget.BaseSkinName));
            }
        }
        ...
        static void Drawing_OnDraw(EventArgs args)
        {
            foreach (var spell in SpellList)
            {
                ...
                {
                    LeagueSharp.Common.Utility.DrawCircle(ObjectManager.Player.Position, spell.Range, menuItem.Color);
                }
            }
        }
