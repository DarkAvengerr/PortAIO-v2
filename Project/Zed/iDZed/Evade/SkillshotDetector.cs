...
using SharpDX;
using SpellData = iDZed.Evade.SpellData;

using EloBuddy; 
 using LeagueSharp.Common; 
 namespace iDZed.Evade
{
    public static class SkillshotDetector
    {
        ...
        private static void GameObject_OnDelete(GameObject sender, EventArgs args)
        {
            ...
            for (var i = ZedEvader.DetectedSkillShots.Count - 1; i >= 0; i--)
            {
                var skillshot = ZedEvader.DetectedSkillShots[i];
                if (skillshot.SpellData.ToggleParticleName != "" &&
                    sender.Name.Contains(skillshot.SpellData.ToggleParticleName))
                {
                    ZedEvader.DetectedSkillShots.RemoveAt(i);
                }
            }
        }
        ...
        private static void ObjSpellMissileOnOnDelete(GameObject sender, EventArgs args)
        {
            ...
            {
                foreach (var skillshot in ZedEvader.DetectedSkillShots)
                {
            ...
            ZedEvader.DetectedSkillShots.RemoveAll(
                skillshot =>
