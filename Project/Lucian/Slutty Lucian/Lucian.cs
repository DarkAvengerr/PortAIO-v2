 ...
 namespace Slutty_Lucian
{
    internal class Lucian : Helper
    {
        ...
        private static void Spellbook_OnCastSpell(Spellbook sender, SpellbookCastSpellEventArgs args)
        {
            ...
            {
                passive = true;
                LeagueSharp.Common.Utility.DelayAction.Add(150, () => passive = false);
            }
            ...
            switch (Orbwalker.ActiveMode)
            {
                case Orbwalking.OrbwalkingMode.Combo:
                    if (ValidTarget(R.Range))
                    {
                       ...
                       LeagueSharp.Common.Utility.DelayAction.Add(1000, () =>  ECast(targets, "useec"));
                    }
                    break;
        ...
        public static void RCast(AIHeroClient target, string name)
        {
        ...
        public static float RDamage(AIHeroClient hero)
        {
            double damage = 0;
        ...
        public static void ExtendedQ(AIHeroClient target, string menuname, string smart)
        {
            if (Player.HasBuff("lucianpassivebuff") || Player.IsDashing() || Player.Spellbook.IsAutoAttacking) return;
        ...
        public static void WCast(AIHeroClient target, string name)
        {
            if (!GetBool(name, typeof (bool))) return;
            if (Player.HasBuff("lucianpassivebuff") || Player.IsDashing() || Player.Spellbook.IsAutoAttacking) return;
        ...
        public static void ECast(AIHeroClient target, string name)
        {
            if (!GetBool(name, typeof (bool))) return;
            ...
            if (casted)
            {
                LeagueSharp.Common.Utility.DelayAction.Add(1000, () => casted = false);
            }
        }
    }
}
