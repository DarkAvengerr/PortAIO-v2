 ...
 namespace VeigSPrediction
{
    ...
    public static class Prediction
    {
        ...
        {
            VeigSPrediction.PathTracker.Initialize();
            VeigSPrediction.Collision.Initialize();
        ...
        internal static Result GetImmobilePrediction(Obj_AI_Base target, float width, float delay, float missileSpeed, float range, bool collisionable, SkillshotType type, Vector2 from)
        {
            ...
            if (target is AIHeroClient)
                result.HitChance = GetHitChance(t - Utility.LeftImmobileTime(target), ((AIHeroClient)target).AvgMovChangeTime(), 0, 0);
            else
                result.HitChance = HitChance.High;
