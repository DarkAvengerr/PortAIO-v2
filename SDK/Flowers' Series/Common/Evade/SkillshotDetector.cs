    ...
    internal static class SkillshotDetector
    {
        ...
        private static void ObjSpellMissileOnOnDelete(GameObject sender, EventArgs args)
        {
            ...
            if (OnDeleteMissile != null)
            {
                ...
                {
                    if (skillshot.SpellData.MissileSpellName.Equals(spellName, StringComparison.InvariantCultureIgnoreCase) && (skillshot.Unit.NetworkId == caster.NetworkId && (missile.EndPosition.ToVector2() - missile.StartPosition.ToVector2()).AngleBetween(skillshot.Direction) < 10) && skillshot.SpellData.CanBeRemoved)
                    {
