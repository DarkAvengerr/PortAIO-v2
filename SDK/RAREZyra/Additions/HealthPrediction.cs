 ...
 namespace RAREZyra.Additions
{
    public class HealthPrediction
    {
        ...
        {
            if (sender.Team != ObjectManager.Player.Team || !Extensions.IsValidTarget(sender, 3000, false) || !(args.Target is Obj_AI_Base))
            {
                return;
            }
        ...
        public static float GetHealthPrediction(Obj_AI_Base unit, int time, int delay = 70)
        {
            ...
            {
                var attackDamage = 0f;
                if (!attack.Processed && Extensions.IsValidTarget(attack.Source, float.MaxValue, false) &&
                    Extensions.IsValidTarget(attack.Target, float.MaxValue, false) && attack.Target.NetworkId == unit.NetworkId)
                {
        ...
        public static float LaneClearHealthPrediction(Obj_AI_Base unit, int time, int delay = 70)
        {
            ...
            foreach (var attack in ActiveAttacks.Values)
            {
                ...
                if (Utils.GameTimeTickCount - 100 <= attack.StartTick + attack.AnimationTime &&
                    Extensions.IsValidTarget(attack.Target, float.MaxValue, false) &&
                    Extensions.IsValidTarget(attack.Source, float.MaxValue, false) && attack.Target.NetworkId == unit.NetworkId)
                {
