 ...
 namespace SoloVayne.Skills.Tumble
{
    class EPositionProvider
    {
        ...
        public Vector3 GetEPosition()
        {
            ...
            var enemiesNear =
                HeroManager.Enemies.Where(m => m.LSIsValidTarget(Orbwalking.GetRealAutoAttackRange(m) + 450f + 65f)).ToList();
            #endregion
            ...
            if (ObjectManager.Player.LSCountEnemiesInRange(1500f) <= 1)
            {
                ...
                var backwardsPosition = (ObjectManager.Player.ServerPosition.LSTo2D() + 450f * ObjectManager.Player.Direction.LSTo2D()).To3D();
            ...
            {
                return ObjectManager.Player.ServerPosition.LSExtend(Game.CursorPos, 450f).IsSafeEx()
