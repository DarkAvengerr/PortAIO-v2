...
using myWorld.Library.SimpleEvade.Pathfinding;
using LeagueSharp;
 ...
 namespace myWorld.Library.SimpleEvade
{
    class Initialization
    {
        ...
        public static bool InitialRandomSpellDodge = false;
        ...
        private static void Game_OnOnGameUpdate(EventArgs args)
        {
            ...
            if (!InitialRandomSpellDodge && Config.Menu.Item("ChooseMode").GetValue<KeyBind>().Active)
            {
                //Chat.Print("Good");
                foreach(Skillshot skillshot in DetectedSkillshots)
                {
                    skillshot.SetRandomValue();
                }
                InitialRandomSpellDodge = true;
            }
            else if(InitialRandomSpellDodge && !Config.Menu.Item("ChooseMode").GetValue<KeyBind>().Active)
            {
                //Chat.Print("Reset");
                InitialRandomSpellDodge = false;
            }
            ...
            if (FollowPath && !NoSolutionFound && (Keepfollowing || !Evading) && EvadeToPoint.IsValid() && safeResult.IsSafe)
            {
                if (EvadeSpellDatabase.Spells.Any(evadeSpell => evadeSpell.Name == "Walking" && evadeSpell.Enabled))
                {
                    ...
                    {
                        var candidate = Pathfinding.Core.PathFind(PlayerPosition, EvadeToPoint);
        ...
        private static void TryToEvade(List<Skillshot> HitBy, Vector2 to)
        {
            ...
            {
                dangerLevel = Math.Max(dangerLevel, skillshot.GetDangerPoint());
            }
            foreach (var evadeSpell in EvadeSpellDatabase.Spells)
            {
