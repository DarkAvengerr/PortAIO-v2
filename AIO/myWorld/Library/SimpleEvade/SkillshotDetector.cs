 ...
 namespace myWorld.Library.SimpleEvade
{
    internal static class SkillshotDetector
    {
        ...
        private static void Game_OnWndProc(WndEventArgs args)
        {
            if (args.Msg == (uint)WindowsMessages.WM_KEYUP)
            {
                TriggerOnDetectSkillshot(
                    ...
                    Initialization.PlayerPosition, Game.CursorPos.To2D(), ObjectManager.Player);
            }
        }
        ...
        private static void GameObject_OnDelete(GameObject sender, EventArgs args)
        {
            ...
            for (var i = Initialization.DetectedSkillshots.Count - 1; i >= 0; i--)
            {
                var skillshot = Initialization.DetectedSkillshots[i];
                if (skillshot.SpellData.ToggleParticleName != "" && new Regex(skillshot.SpellData.ToggleParticleName).IsMatch(sender.Name))
                {
                    Initialization.DetectedSkillshots.RemoveAt(i);
                }
            }
        }
        ...
        private static void ObjSpellMissileOnOnDelete(GameObject sender, EventArgs args)
        {
            ...
            Initialization.DetectedSkillshots.RemoveAll(
                skillshot =>
