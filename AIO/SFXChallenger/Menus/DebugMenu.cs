...
using EloBuddy; namespace SFXChallenger.Menus
{
                ...
                _menu.AddItem(new MenuItem(_menu.Name + ".reset", "Reset").SetValue(false).DontSave()).ValueChanged +=
                    delegate(object sender, OnValueChangeEventArgs args)
                    {
                        try
                        {
                            ...
                            {
                                LeagueSharp.Common.Utility.DelayAction.Add(0, () => _menu.Item(_menu.Name + ".reset").SetValue(false));
                ...
                _menu.AddItem(new MenuItem(_menu.Name + ".report", "Generate Report").SetValue(false)).ValueChanged +=
                    delegate(object sender, OnValueChangeEventArgs args)
                    {
                        try
                        {
                            ...
                            LeagueSharp.Common.Utility.DelayAction.Add(0, () => _menu.Item(_menu.Name + ".report").SetValue(false));
                            File.WriteAllText(
