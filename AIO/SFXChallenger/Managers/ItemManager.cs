...
using EloBuddy; namespace SFXChallenger.Managers
{
    ...
    public class ItemManager
    {
        ...
        public static void UseComboItems(AIHeroClient target, bool killSteal = false)
        {
            if (_menu == null || !_menu.Item(_menu.Name + ".enabled").GetValue<bool>())
            {
                return;
        ...
        public static void Muramana(AIHeroClient target, bool activate, float overrideRange = -1f)
        {
            try
            {
        ...
        public static float CalculateLichBaneDamage(AIHeroClient target)
        {
            try
            {
                if (target == null)
