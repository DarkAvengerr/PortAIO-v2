    ...
    public class Projections
    {
        ...
        private static void MissileClient_OnSpellMissileCreate(GameObject sender, EventArgs args)
        {
            ...
            if (missile.SpellCaster is AIHeroClient && missile.SpellCaster?.Team != Player.Team)
            {
                ...
                if (startPos.LSDistance(endPos) > data.CastRange)
                    endPos = startPos + direction * data.CastRange;

                if (startPos.LSDistance(endPos) < data.CastRange && data.FixedRange)
                    endPos = startPos + direction * data.CastRange;
                ...
                foreach (var hero in Activator.Allies())
                {
                    ...
                    var distance = (1000 * (startPos.LSDistance(hero.Player.ServerPosition) / data.MissileSpeed));
                ...
                if (attacker.IsValid<AIHeroClient>())
                {
                    foreach (var hero in Activator.Allies())
                    {
                            ...
                            args.SData.TargettingType == SpellDataTargetType.SelfAoe)
                        {
                            ...
                            if (hero.Player.LSDistance(correctpos) > data.CastRange + 125)
                                continue;

                            if (data.SDataName == "kalistaexpungewrapper" && !hero.Player.LSHasBuff("kalistaexpungemarker"))
                                continue;
                            ...
                            args.SData.TargettingType.ToString().Contains("Location"))
                        {
                            ...
                            if (hero.Player.LSDistance(startpos) > data.CastRange + 35)
                                continue;

                            if ((data.SDataName == "azirq" || data.SDataName == "azire") && fromobj == null)
                                continue;

                            var distance = (int) (1000 * (startpos.LSDistance(hero.Player.ServerPosition) / data.MissileSpeed));
                            ...
                            if (startpos.LSTo2D().LSDistance(endpos) > data.CastRange)
                                endpos = startpos.LSTo2D() + direction * data.CastRange;

                            if (startpos.LSTo2D().LSDistance(endpos) < data.CastRange && data.FixedRange)
                                endpos = startpos.LSTo2D() + direction * data.CastRange;
                            ...
                            if (!isline || iscone)
                                 evadetime =
                                     (int) (1000 * (data.Radius - hero.Player.LSDistance(startpos) + hero.Player.BoundingRadius) / hero.Player.MoveSpeed);

                            if (isline && data.Radius + hero.Player.BoundingRadius + 35 > projdist ||
                               (!isline || iscone) && hero.Player.LSDistance(endpos) <= data.Radius + hero.Player.BoundingRadius + 35)
                            {
                                if (data.CastRange > 10000)
                                {
                                    if (hero.Player.NetworkId == Player.NetworkId)
                            ...
                            args.SData.TargettingType == SpellDataTargetType.SelfAndUnit)
                        {
                            ...
                            if (hero.Player.LSDistance(attacker.ServerPosition) > data.CastRange + 100)
                                continue;

                            var distance =
                                (int) (1000 * (attacker.LSDistance(hero.Player.ServerPosition) / data.MissileSpeed));
        ...
        private static void AIHeroClient_OnPlayAnimation(Obj_AI_Base sender, GameObjectPlayAnimationEventArgs args)
        {
            ...
            if (aiHero.ChampionName == "Jax" && aiHero.IsEnemy)
            {
                if (args.Animation == "Spell3")
                {
                    ...
                    {
                        if (aiHero.LSHasBuff("JaxCounterStrike"))
                        {
